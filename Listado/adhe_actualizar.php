...
<form action="adhe_update.php" method="POST">
            <?php 
    $conexion = mysqli_connect("localhost","root","","remis");
    $id= $_GET["id"];
    $adherentes ="SELECT * FROM adherentes WHERE id='$id'";

    $resultado=mysqli_query($conexion,$adherentes);
     
     while($row=mysqli_fetch_assoc($resultado)) {?>
            <h1>Datos de Adherentes</h1>
            <input type="hidden" name="id" value="<?php echo $row['id'] ?>">

        <input type="text" class="form-control mb-3" name="adhe_descripcionRTO" placeholder="adhe_descripcionRTO" value="<?php echo $row['adhe_descripcionRTO'];?>">
        <input type="text" class="form-control mb-3" name="adhe_habilitacion" placeholder="adhe_habilitacion" value="<?php echo $row['adhe_habilitacion'];?>">
